...
let wf_canvas, wf_ctx, wf_analyser;
...
function initAudioVisualizer(audioObj, canvasObj){
	...
	const source = context.createMediaElementSource(audioObj); 
...
function frameLooper(canvas){
	...
	const fbc_array = new Uint8Array(wf_analyser.frequencyBinCount);	
	...
	for(let i = 0; i < 16; i++) {
		const bar_x = i * wf_canvas.width/32.0;	// x offset
		const bar_width = (wf_canvas.width/32.0/4);	// y offset
		const bar_height = -(fbc_array[fbc_size * i] * wf_canvas.height / 510.0);
		//  fillRect( x, y, width, height ) 
		wf_ctx.fillRect(wf_canvas.width/2 + bar_x, 
      wf_canvas.height/2, bar_width, bar_height);	// 1사분면
		wf_ctx.fillRect(wf_canvas.width/2 + bar_x, 
      wf_canvas.height/2, bar_width, -bar_height);	// 2사분면
		wf_ctx.fillRect(wf_canvas.width/2 - bar_x + bar_width - wf_canvas.width/32.0, 
      wf_canvas.height/2, -bar_width, bar_height);	// 3사분면
		wf_ctx.fillRect(wf_canvas.width/2 - bar_x + bar_width - wf_canvas.width/32.0, 
      wf_canvas.height/2, -bar_width, -bar_height);	// 4사분면
	}
}